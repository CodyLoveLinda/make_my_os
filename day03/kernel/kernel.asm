...
#diplay something
  movw $msg,%si
    c431:	c4                   	(bad)  
  call puts
    c432:	e8 5d 00 be 81       	call   81bec494 <__bss_start+0x81bdff14>
  
  movw $try,%si
    c437:	c4                   	(bad)  
  call puts
    c438:	e8 57 00 fa fc       	call   fcfac494 <__bss_start+0xfcf9ff14>
...
0000c481 <try>:
    ...
    c486:	72 79                	jb     c501 <clear_screen+0x1d>
...
void bootmain(void){
    ...
    c4e2:	eb fe                	jmp    c4e2 <bootmain+0x4>

0000c4e4 <clear_screen>:
#include<header.h>

void clear_screen(char color) //15:pure white
{
    c4e4:	f3 0f 1e fb          	endbr32 
    c4e8:	55                   	push   %ebp
    c4e9:	89 e5                	mov    %esp,%ebp
    c4eb:	0f b6 55 08          	movzbl 0x8(%ebp),%edx
  int i;
  for(i=0xa0000;i<0xaffff;i++)
    c4ef:	b8 00 00 0a 00       	mov    $0xa0000,%eax
  {
  write_mem8(i,color);  //if we write 15 ,all pixels color will be white,15 mens pure white ,so the screen changes into white
    c4f4:	88 10                	mov    %dl,(%eax)
  for(i=0xa0000;i<0xaffff;i++)
    c4f6:	83 c0 01             	add    $0x1,%eax
    c4f9:	3d ff ff 0a 00       	cmp    $0xaffff,%eax
    c4fe:	75 f4                	jne    c4f4 <clear_screen+0x10>
...
0000c502 <color_screen>:

void color_screen(char color) //15:pure white
{
    c502:	f3 0f 1e fb          	endbr32 
  int i;
  color=color;
  for(i=0xa0000;i<0xaffff;i++)
    c506:	b8 00 00 0a 00       	mov    $0xa0000,%eax
  {
  write_mem8(i,i&0x0f);  //if we write 15 ,all pixels color will be white,15 mens pure white ,so the screen changes into white
    c50b:	89 c2                	mov    %eax,%edx
    c50d:	83 e2 0f             	and    $0xf,%edx
    c510:	88 10                	mov    %dl,(%eax)
  for(i=0xa0000;i<0xaffff;i++)
    c512:	83 c0 01             	add    $0x1,%eax
    c515:	3d ff ff 0a 00       	cmp    $0xaffff,%eax
    c51a:	75 ef                	jne    c50b <color_screen+0x9>
